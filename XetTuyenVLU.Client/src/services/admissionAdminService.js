...
export async function GetDataForBangDiem() {
  return await axios.get(API_URL + "GetDataForBangDiem", {
    headers: {
      Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
    },
  });
...
export async function RejectAdmissionProfileById(id) {
  return await axios.get(API_URL + `RejectAdmissionProfileById/${id}`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
    },
  });
...
export async function UpdateSendMailStatusById(id) {
  return await axios.get(API_URL + `UpdateSendMailStatusById/${id}`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem("jwtToken")}`,
    },
  });
