...
import { ValidateCMNDEdit } from "../../services/profileService";
...
function Edit(props) {
  ...
  useEffect(() => {
    ...
    const dataSession = JSON.parse(sessionStorage.getItem("dataEdit"));
  ...
  const onGetDataForDropdownList = useCallback(() => {
    ...
    //Phuong an
    if (data.phuongAn) {
      setPhuongAn(data.phuongAn.toString());
    }
    ...
    //Set nganh 
    GetNganhXetTuyen().then((response) => setIsSpecialProgram1(response.data.find((x) => x.maNganh === data.nganh1).ctdb !== null ? true : false));
    if (data.nganh2 !== "") {
      GetNganhXetTuyen().then((response) => setIsSpecialProgram2(response.data.find((x) => x.maNganh === data.nganh2).ctdb !== null ? true : false));
    }
    if (data.nganh3 !== "") {
      GetNganhXetTuyen().then((response) => setIsSpecialProgram3(response.data.find((x) => x.maNganh === data.nganh3).ctdb !== null ? true : false));
    }
  ...
  const onReview = (data) => {
    ...
    sessionStorage.setItem("dataEdit", JSON.stringify(data));
    props.setDataEdit(data);
    console.log(data);
    navigate("/previewEdit", { replace: true });
  };
  ...
  return (
    <Container className={classes.container}>
      <div className={classes.containerTitle}>
        <div className={classes.title}>ĐIỀU CHỈNH THÔNG TIN XÉT TUYỂN NĂM {schedule.namXetTuyen} - HỌC BẠ ĐỢT {phase.thuTuDot}</div>
        <div>
          <span className={classes.subTitle}>
            (Lưu ý:
          </span>
          <span className={classes.subTitle2} >Thí sinh chỉ được điều chỉnh từ ngày {schedule.ngayBatDau} - {schedule.ngayKetThuc}. Bạn cần căn nhắc trước khi điều chỉnh.</span>
          <span className={classes.subTitle}>
            )
          </span>
        </div>
        ...
        <FormGroup>
          <Row>
            ...
            <Col md={3} style={{ paddingLeft: "25px" }}>
              <Input
                ...
                {...register("cmnd", {
                  ...
                  validate: {
                    value: async (value) =>
                      (await ValidateCMNDEdit(value, data.cmnd)).data == false ||
            ...
            <Col md={3}>
              ...
              {errors && (
                <div className={classes.error}>{errors.quoctich?.message}</div>
              )}
            </Col>
          </Row>
        </FormGroup>
        ...
        <FormGroup>
          <Row>
            ...
            <Col md={3} style={{ paddingLeft: "25px" }}>
              ...
              {errors && (
                <div className={classes.error}>{errors.quanhuyen?.message}</div>
              )}
            </Col>
          </Row>
        </FormGroup>
        <FormGroup>
          <Row>
            ...
            <Col md={3} style={{ paddingLeft: "25px" }}>
              ...
              {errors && (
                <div className={classes.error}>{errors.phuongxa?.message}</div>
              )}
            </Col>
          </Row>
        </FormGroup>
        ...
        <FormGroup>
          <Row>
            ...
            <Col md={5} style={{ paddingLeft: "25px" }}>
              ...
              />
              {errors && (
                <div className={classes.error}>{errors.diachinha?.message}</div>
              )}
            </Col>
          </Row>
        </FormGroup>
            ...
            >
              <div style={{ display: "flex", alignItems: "center" }}>
                <Input
                  ...
                  {...register("phuongAn", { required: { value: true, message: "Phương án là bắt buộc" } })}
                />
        ...
        <FormGroup>
          ...
          >
            ...
            <Col md={3}>
              ...
              {errors && (
                <div className={classes.error}>{errors.tohopmon1?.message}</div>
              )}
            </Col>
              ...
              <Input
                ...
                {...register("nganh2", {
                  onChange: (e) => {
                    ...
                    const majorSelected = nganh2.find(
                      (x) => x.maNganh === e.target.value

                    );

                    setIsSpecialProgram2(
                      majorSelected.ctdb !== null ? true : false
                    );
                    onGetToHopXetTuyen(e.target.value)
                      .then((response) => {
                        setToHop2(response.data);
              ...
              <Input
                ...
                {...register("nganh3", {
                  onChange: (e) => {
                    ...
                    const majorSelected = nganh3.find(
                      (x) => x.maNganh === e.target.value
                    );
